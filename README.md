...
> **随时随地，便捷召唤 AI！这里集中维护 Anywhere Desktop 的官方用户指南，覆盖快捷助手、Better Work、全局追问、MCP、Skill、定时任务、项目化历史对话、WebDAV 同步、模型服务商与桌面端专属配置。**
...
本次文档同步覆盖 Anywhere Desktop `v1.2.0` 的主要新增能力：

* **Better Work 交互工具**：对话内选择卡片、自由输入 / 讨论选项、会话临时任务面板、任务进度保存与恢复。
* **消息缓冲机制**：请求进行中继续发送消息时自动排队，当前回复或工具调用完成后再续接执行。
* **聊天项目管理**：历史对话支持项目分组、拖拽归档、批量拖拽、本地与云端项目归属同步。
* **截图标注工具**：截图后可直接画笔 / 矩形标注，再发送给图片类助手。
* **模型协议与请求配置**：新增 Codex API、Claude / Anthropic API 协议，以及 Provider 自定义 Header。
* **Token 用量展示**：助手回复可展示输入 / 输出 Token 与推理 Token。
...
### 💬 独立对话窗口、Better Work 与历史恢复

桌面端独立窗口支持多轮对话、文件拖拽、图片粘贴、Token 用量展示、自动保存、导出与恢复。通过 Better Work，AI 还可以在对话中展示单选确认卡片、收集自由输入、维护临时任务面板，并在长任务中实时同步执行状态。
...
* **MCP**：让 AI 拥有文件、Shell、Python、联网、Better Work 交互与任务管理能力；
* **Skill**：把复杂流程封装为 SOP；
...
* **想玩自动化 / 长任务协作**：MCP 服务 → Better Work → Skill 技能库 → 定时任务
* **想做多端同步与项目化归档**：设置与同步 → 历史对话
...
| **历史对话**     | 管理本地与云端会话记录，支持项目分组、恢复、清理与导出。 | [查看文档](./docs/chat_doc.md)     |
| **快捷助手**     | 创建不同类型的助手，掌握快捷召唤、全局追问、消息缓冲与截图标注。 | [查看文档](./docs/ai_doc.md)       |
| **MCP 服务**     | 启用内置工具、Better Work 交互工具，并接入第三方 MCP 服务。 | [查看文档](./docs/mcp_doc.md)      |
| **Skill 技能库** | 编写 SOP、封装技能、使用子智能体模式。         | [查看文档](./docs/skill_doc.md)    |
| **服务商管理**   | 配置 API 服务商、模型协议、多 Key 轮询与自定义 Header。 | [查看文档](./docs/provider_doc.md) |
